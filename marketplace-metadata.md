...
- **필요 환경변수**: `KR_FINANCE_LICENSE_KEY` (선택, 프리미엄 도구 4종에만 필요)
...
| Config (env vars) | `KR_FINANCE_LICENSE_KEY` (optional, string) |
  ...
  configSchema:
    ...
    properties:
      KR_FINANCE_LICENSE_KEY:
        type: string
        description: "프리미엄 도구(양도소득세/DSR·DTI/환율변환/청약가점) 사용을 위한 라이선스 키 (선택)"
    required: []
  commandFunction: |
    (config) => ({
      command: "npx",
      args: ["-y", "mcp-finance-tools"],
      env: config.KR_FINANCE_LICENSE_KEY
